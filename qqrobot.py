...
class QQClient():
    ...
    def __init__(self, cmdr=None, HTTPClient=mHTTPClient_gevent, handlers=[]):
        self.cmdr = cmdr
        self.friend_list = QQFriends()
        self.http_client = HTTPClient(cmdr)
        ...
        try:
            ...
            if resp.get('retcode', 0) != 0:
                ...
                log.e(tag, 'error retcode %d errmsg %s' % (
                    resp.get('retcode', 0), resp.get('errmsg', 'none')), cmdr=self.cmdr)
                if resp.get('retcode', 0) == 103:
                    # Connection failed, log in again.
                    log.w(tag, 'Meet with error 103.', cmdr=self.cmdr)
                    log.w(tag, 'You\'ll have to log in again.', cmdr=self.cmdr)
                    exit()
            else:
                if resp.get('retcode') != 0:
                    # something is really wrong
                    log.e(tag, 'retcode %s errmsg %s' % (
                        resp.get('errmsg'),resp.get('retcode')), cmdr=self.cmdr)
                    return
            ...
            log.e(tag, 'Fatal error parsing messages.', cmdr=self.cmdr)
            log.e(tag, 'Response: ' + str(resp), cmdr=self.cmdr)
            traceback.print_exc()
    ...
    def _callback_send(self, resp, previous):
        ...
        if resp.get('errCode', 0) != 0 or resp.get('retcode', 0) != 0:
            log.w(tag, 'The following error occurred when sending last message:', cmdr=self.cmdr)
            if resp.get('retcode', 0) == 1202:
                # retcode 1202: ignored, as WebQQ itself has ignored it too.
                log.w(tag, '    retcode 1202', cmdr=self.cmdr)
                log.w(tag, '    Message could be lost, but usually not big deal.', cmdr=self.cmdr)
    ...
    def QR_veri(self, show_QR=None):
        ...
        if show_QR is None:
            def e():
                ...
                if s == 'Darwin': # Mac OSX
                    ...
                    exec_cmd('rifle ' + f)
                log.i(tag, 'QR Image saved @ ' + f, cmdr=self.cmdr)
            show_QR = e
        show_QR()
        ...
        while True:
            ...
            if prev != res[0]:
                if res[0] == '65':
                    log.i(tag, 'QR code expired.', cmdr=self.cmdr)
                    show_QR()
                elif res[0] == '66':
                    log.i(tag, 'Please scan the QRCode shown on your screen.', cmdr=self.cmdr)
                elif res[0] == '67':
                    log.i(tag, 'Please press confirm on your phone.', cmdr=self.cmdr)
                elif res[0] == '0':
                    # QR code verification completed
                    log.i(tag, res[-2], cmdr=self.cmdr)
                    self.username = res[-1]
                    log.i(tag, 'User name: ' + self.username, cmdr=self.cmdr)
                    break
                prev = res[0]
    ...
    def login(self, get_info=True, save_veri=False, filename=None):
        ...
        if save_veri:
            log.i(tag, 'Verification saved @ ' + self.save_veri(filename), cmdr=self.cmdr)
    ...
    def load_veri(self, filename):
        ...
        log.i(tag, 'Verification loaded from ' + filename, cmdr=self.cmdr)
        log.i(tag, 'Username: ' + self.username, cmdr=self.cmdr)
    ...
    def listen(self, join=False):
        ...
        def l():
            log.i('listener', 'Listener thread started.', cmdr=self.cmdr)
            while True:
    ...
    def get_user_friends(self):
        ...
        log.i('list', 'Finished getting friend list.', cmdr=self.cmdr)
    ...
    def get_group_list(self):
        self.friend_list.parse_groups(self.http_client.get_json(
            ...
            data={'r': json.dumps({
                  ...
                  'vfwebqq': self.vfwebqq})},
            headers=self.default_headers))
        log.i('list', 'Group list fetched.', cmdr=self.cmdr)
    ...
    def get_discus_list(self):
        self.friend_list.parse_discus(self.http_client.get_json(
            ...
            data={'clientid': 53999199,
                  ...
                  't': utime()},
            headers=self.default_headers))
        log.i('list', 'Discus group list fetched.', cmdr=self.cmdr)
    ...
    def get_online_buddies(self):
        ...
        log.i('list', 'Online buddies list fetched.', cmdr=self.cmdr)
    ...
    def get_recent_list(self):
        ...
        log.i('list', 'Recent list fetched.', cmdr=self.cmdr)
    ...
    def get_self_info(self):
        ...
        if not hasattr(self, 'info'):
            ...
            if r['retcode'] == 0:
                ...
                log.e('info', 'User self info fetching failed.', cmdr=self.cmdr)
        return self.info
